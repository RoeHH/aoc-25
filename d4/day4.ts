...
const driveTheForklift = (grid: boolean[][], resultGrid: number[][]): void => {
  for (let y = 0; y < grid.length; y++) {
    for (let x = 0; x < grid[0].length; x++) {
      if (grid[y][x]) {
        resultGrid[y][x] = saveCheck(grid, x - 1, y - 1) +
          saveCheck(grid, x, y - 1) + saveCheck(grid, x + 1, y - 1) +
          saveCheck(grid, x - 1, y) + saveCheck(grid, x + 1, y) +
          saveCheck(grid, x - 1, y + 1) + saveCheck(grid, x, y + 1) +
          saveCheck(grid, x + 1, y + 1);
      }
    }
  }
};
...
while (true) {
  ...
  console.log(
    resultGrid.map((row) =>
      row.map((v) => v === -1 ? "🔳" : v < 4 ? "✅" : "❌").join("")
    ).join("\n"),
  );
  console.log(
    "Result after drive:",
    resultGrid.flat().filter((v) => v >= 0 && v < 4).length,
  );
